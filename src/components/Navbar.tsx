...
const Navbar = () => {
    ...
    console.log(session?.user, "this is it");
  return (
    <div className="border-b border-purple-500 py-2 px-10 flex justify-between items-center dark:bg-black dark:text-white">
        ...
        <div className="pt-2">
            {session?.user ? <><PrimaryButton onClick={() => {signOut(); console.log(session?.user,"this is not it");}}>Username </PrimaryButton></> : <><PrimaryButton onClick={()=>signIn("google")}>Login</PrimaryButton></>}
        </div>
    </div>
  )
}
